-- ============================================
-- SCRIPT DE MIGRACIÓN SEGURA DE BASE DE DATOS
-- Elimina foreign keys problemáticas antes de recrear tablas
-- ============================================
...
-- ============================================
-- ETAPA 1: Eliminar foreign keys problemáticas
-- ============================================
-- Eliminar foreign keys de tablas que referencian orden_item
DROP TABLE IF EXISTS orden_item_modificador;
DROP TABLE IF EXISTS preparacion_orden;
DROP TABLE IF EXISTS orden_item;

-- Eliminar tablas antiguas que pueden causar conflictos
DROP TABLE IF EXISTS elementos_cuenta;
DROP TABLE IF EXISTS cuentas;
DROP TABLE IF EXISTS cierres_caja;

-- Eliminar otras tablas relacionadas que pueden tener conflictos
...
-- Eliminar tablas de productos que pueden tener conflictos
...
-- Eliminar tablas de mesas
DROP TABLE IF EXISTS mesa_estado_hist;
DROP TABLE IF EXISTS reserva;
DROP TABLE IF EXISTS mesa;

-- Eliminar tablas de clientes
DROP TABLE IF EXISTS cliente;

-- Eliminar tablas de caja
...
-- Eliminar tablas de alertas (si existe la antigua estructura)
DROP TABLE IF EXISTS alerta;

-- Eliminar tablas de roles (si existen con estructura diferente)
...
-- Eliminar tabla de historial de passwords
...
-- ============================================
-- La tabla usuario se mantiene (ya existe y tiene datos)
...
CREATE TABLE IF NOT EXISTS orden_item (
  ...
  CONSTRAINT fk_oi_tamano FOREIGN KEY (producto_tamano_id) REFERENCES producto_tamano(id) ON UPDATE CASCADE ON DELETE SET NULL
) ENGINE=InnoDB;
...
-- ============================================
-- ETAPA 11: Seeds mínimos (roles, permisos, estados, formas pago)
-- ============================================
INSERT IGNORE INTO rol (id, nombre, descripcion) VALUES
  (1, 'administrador', 'Acceso total'),
  (2, 'cajero', 'Caja y pagos'),
  (3, 'capitan', 'Coordina sala'),
  (4, 'mesero', 'Toma órdenes'),
  (5, 'cocinero', 'KDS cocina');
...
-- ============================================
-- ETAPA 12: Asignar rol de administrador al usuario admin existente
-- ============================================
INSERT IGNORE INTO usuario_rol (usuario_id, rol_id)
SELECT u.id, 1 FROM usuario u WHERE u.username = 'admin' LIMIT 1;
